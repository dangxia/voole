...
public class CamusMultiOutputFormat extends FileOutputFormat<CamusKey, Object> {
	...
	private volatile TopicMeta topicMeta;
	private volatile ICamusPartitioner partitioner;
	...
	public TopicMeta getTopicMeta(TaskAttemptContext context, String topic) {
		if (topicMeta == null) {
			initTopicMeta(context, topic);
		}
		return topicMeta;
	}

	private synchronized void initTopicMeta(TaskAttemptContext context,
			String topic) {
		if (topicMeta != null) {
			return;
		}
	...
	public ICamusPartitioner getPartitioner(TaskAttemptContext context,
			String topic) {
		if (partitioner == null) {
			initPartitioner(context, topic);
		}
		return partitioner;
	}

	private synchronized void initPartitioner(TaskAttemptContext context,
			String topic) {
		if (partitioner != null) {
			return;
		}
		partitioner = CamusMetaConfigs.getPartitionerManager(context)
				.findPartitioner(topic);
	}
	...
	class CamusMultiRecordWriter extends RecordWriter<CamusKey, Object> {
		...
		private RecordWriter<AvroKey<SpecificRecordBase>, NullWritable> createDataRecordWriter(
			...
			return new AvroKeyRecordWriter<SpecificRecordBase>(getTopicMeta(
					context, key.getTopic()).getSchema(),
