 ...
 * @date 2014年7月29日
 */
public class HiveDirectInputFileFilter {
	private Logger log = LoggerFactory
			.getLogger(HiveDirectInputFileFilter.class);
	...
	public HiveDirectInputFileFilter(JobContext job) {
		prevCamusExecTime = HiveDirectMetaConfigs.getPrevCamusExecTime(job);
		camusDestPath = HiveDirectMetaConfigs.getCamusDestPath(job).toUri()
				.getPath();
	...
	public boolean accept(Path path) {
		...
		if (isInCamsuDescPath(path)) {
			...
			if (name.startsWith(HiveDirectMetaConfigs.NOEND_PREFIX)) {
