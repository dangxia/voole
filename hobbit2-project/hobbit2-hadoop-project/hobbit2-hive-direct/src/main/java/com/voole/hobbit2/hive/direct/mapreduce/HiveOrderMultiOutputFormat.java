		...
		public void write(Object key, Object value) throws IOException,
			...
			if (key instanceof HiveTable) {// 正常值
				...
				if (value != null && value instanceof String
					...
					createTransformeExceptionWriter((String) key).append(
							NullWritable.get(), transformExceptionText);
				}
		...
		public void close(TaskAttemptContext context) throws IOException,
			...
			if (fileNameToHiveTableMap.size() > 0) {
				...
				String fileName = getUniqueFile(context,
						HiveDirectMetaConfigs.FILE_INFO_PREFIX, ".fileInfo");
				Path path = new Path(workPath, fileName);

				HiveDirectHDFSUtils.writeFileNameToHiveTableMap(
